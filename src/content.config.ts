...
const expeditions = defineCollection({
  loader: glob({
    ...
    // Sem isso, o id gerado seria "vale-da-lua-canions/index" (o loader não
    // colapsa "/index" sozinho como o sistema de collections antigo fazia).
    // Isso garante que o id continue exatamente igual ao slug de antes,
    // então as URLs (/expedicoes/vale-da-lua-canions/) não mudam.
    generateId: ({ entry }) => entry.replace(/\/index\.md$/, ""),
  }),
  // O helper `image()` só existe quando o schema é uma função — ele resolve
  // o caminho de `cover` (ex: "./cover.jpg") pro arquivo de imagem de
  // verdade, colocado ao lado do index.md, e permite usar <Image>/<Picture>
  // do astro:assets com otimização e srcset responsivo automáticos.
  schema: ({ image }) =>
    z.object({
      // Campos obrigatórios: o mínimo pra uma expedição aparecer no site.
      title: z.string(),
      subtitle: z.string(),
      cover: image(), // arquivo ao lado do index.md, ex: "./cover.jpg"
      location: z.string(),
